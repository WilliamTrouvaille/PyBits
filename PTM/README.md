...
# 指定输出目录
./CLI/PTM input.pdf --out-dir /path/to/output

# 启用图片提取
./CLI/PTM input.pdf --images

# 设置超时时间（秒）
./CLI/PTM input.pdf --timeout 600

# 使用代理
./CLI/PTM input.pdf --proxy http://127.0.0.1:7890
```
...
```bash
./CLI/PTM ~/Documents/paper.pdf --images --out-dir ~/Desktop/output
```
...
```bash
./CLI/PTM large_paper.pdf --timeout 1200
```
